...
HWND hWndMain, hWnd2;
HANDLE hThread;
LPCTSTR lpszClass = TEXT("Bounce2"); //윈도우 창의 제목
...
void moveBall();	//원래 moveball() + 벽 튕기기
...
void send() { //데이터 송신
	...
	HWND m_Window = ::FindWindow(NULL, TEXT("Bounce1")); //송신할 창의 이름을 지정
...
void OnTimer(HWND hWnd)
{
	...
	Ellipse(hMemDC, x - R - 500, y - R, x + R - 500, y + R);	 //공그리기
	//공의 실제 좌표는 500~1000이지만, 우리는 그것을0~500으로
	//봐야 공이 좌표 0에서 부터 시작함 그래서 -500만큼의 보정이 들어간다.
...
void moveBall()//상하우가 막혀있음
{
	...
	if (x - 500 >= crt.right - R) {  //우측만 막혀있으니 좌측만 보정, 우측의 실제값은 500~1000이나, 실제창은 그 값을 0~500으로 받아들이기 때문에 -500을 보정해준다.
		xv *= -(BOUNCE);
		x = crt.right - R + 500; //crt값은 실제로 1000인데 crt.right는 500이므로 +500만큼의 보정이 들어가야함
	}
	x += (int)xv;
	y += (int)yv;
	if (x >= 500) { //우측화면에 뜨는 경우가 x가 500이상인 경우기 때문이고, 화면에 나오는 경우에만 정보를 전송하게 하기 위해 위와 같이 코딩
		send();
	}
}
